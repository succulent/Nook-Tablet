...
int InitDisplayManagerMetaData() {
    ...
    */
    ...
    if (error)
        LOGE("There was an error while populating the metadata\n");
...
void overlay_control_context_t::calculateDisplayMetaData(overlay_object *overlayobj, int panelId)
{
    ...
    static const char* panelname = "boxerLCD";
                                        ...
                                        uint32_t w, uint32_t h, int32_t  format, int isS3D)
{
    ...
    if ( (fp = fopen("/data/data/com.android.settings/files/powersavemodeenabled.txt","r") ) != NULL )
      {
        powersavemodeenabled = 1 ;
	fclose(fp);
      }
    else
      powersavemodeenabled = 0 ;

    if ( powersavemodeenabled ) {
    fp = fopen("/sys/devices/omapdss/display0/timings" , "w");
    if (fp != NULL)
      {
	fputs("26285,1024/64/16/10,600/2/2/2", fp);
	fclose(fp);
      }
    else
      {
                                   ...
                                   overlay_t* overlay)
{
	...
	if (fp != NULL)
	  {
	    fputs("46000,1024/160/160/2,600/10/23/10", fp);
	    fclose(fp);
	  }
	else
	  {
...
int overlay_control_context_t::overlay_commit(struct overlay_control_device_t *dev,
    ...
    */
    ...
    /*
     In case of 180 clockwise and anti-clockwise rotations where window is not supposed to change, commit has to be executed right away.
     In all other rotation cases where window has not changed yet, current commit has to be ignored since SurfaceFlinger will set the updated
     window parameters later and call commit again. So, the current commit should be ignored to avoid skewed Video effects observed during rotations.
     */
    if ((abs(data->rotation-stage->rotation) != 180) &&
